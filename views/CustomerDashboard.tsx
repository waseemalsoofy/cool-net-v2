...
const CustomerDashboard: React.FC<DashboardProps> = ({ user, onLogout }) => {
  ...
  const handleBuy = (packageId: string, useWallet: boolean) => {
    setLoading(true);
    try {
      const order = mockAuth.purchaseCard(user.id, packageId, useWallet);
  ...
  const handleBalanceSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    mockAuth.requestDeposit(user.id, Number(balanceRequest.amount), balanceRequest.method, balanceRequest.reference);
        ...
        {activeTab === 'HISTORY' && (
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-2xl shadow-sm overflow-hidden border border-gray-100">
              {mockAuth.transactions.filter(t => t.userId === user.id).length === 0 ? (
                <p className="p-8 text-center text-gray-400">لا توجد معاملات سابقة</p>
              ) : (
                <div className="divide-y divide-gray-100">
                  {mockAuth.transactions.filter(t => t.userId === user.id).map(t => (
                    <div key={t.id} className="p-4 flex justify-between items-center">
                      ...
                      <span className={`px-2 py-1 rounded text-xs font-bold ${
                        t.status === TransactionStatus.APPROVED ? 'bg-green-100 text-green-600' :
